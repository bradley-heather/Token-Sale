...
import qualified Week08.TokenSale                   as Ts
...
instance ContractModel TSModel where
    ...
    nextState (Close v w) = do
        when (v == w) $ do
             ...
             case m of
                 Just t 
                     | t ^. tssToken >= 0 && t ^. tssLovelace >= 0 -> do
                         let allTokens = t ^. tssToken
                         let allLL = t ^. tssLovelace
    ...
    perform h _ cmd = case cmd of
        ...
        (SetPrice v w p)   -> callEndpoint @"interact"   (h $ UseKey v w) (Ts.SetPrice p)                                                    >> delay 1
        (AddTokens v w n)  -> callEndpoint @"interact"   (h $ UseKey v w) (Ts.AddTokens n)                                                   >> delay 1
        (BuyTokens v w n)  -> callEndpoint @"interact"   (h $ UseKey v w) (Ts.BuyTokens n)                                                   >> delay 1
        (Withdraw v w n l) -> callEndpoint @"interact"   (h $ UseKey v w) (Ts.Withdraw n l)                                                  >> delay 1
        (Close v w)        -> callEndpoint @"interact"   (h $ UseKey v w) (Ts.Close)                                                         >> delay 1
